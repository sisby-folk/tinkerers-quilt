    ...
    for tier_idx, current_tier in enumerate(tiers):
        for recipe_type in ['dework', 'dework_final', 'repair', 'repair_final', 'upgrade']:
            ...
            if recipe_type.startswith('upgrade'):
                if current_tier == tiers[0] or type_costs[type_idx] > 5:  # No iron or armor
                    continue
...
tiers = [('wooden', 59), ('stone', 131), ('iron', 250)]
repair = ['planks', 'stone_tool_materials', 'iron_ingot']
repair_type = ['tag', 'tag', 'item']
        ...
        for tier_idx, current_tier in enumerate(tiers):
            for recipe_type in ['repair_single', 'repair_single_final', 'repair_double', 'repair_double_final', 'repair_triple', 'repair_triple_final', 'upgrade']:
                ...
                if recipe_type.startswith('repair'):
                    ...
                    durability = current_tier[1]
                    current_dict['ingredients'][0]['item'] = 'minecraft:' + current_tier[0] + '_' + current_type
                ...
                if recipe_type.startswith('upgrade'):
                    current_dict['ingredients'][0]['item'] = 'minecraft:' + tiers[tier_idx - 1][0] + '_' + current_type
